...
import { Key, Mail, Lock, Sparkles } from "lucide-react";
...
function LoginContent() {
  ...
  // Surface errors redirected from the OAuth/magic-link callback.
  useEffect(() => {
    ...
    >
      ...
      <div className="container-xl relative z-10 max-w-md px-6">
        ...
        >
          ...
          <form onSubmit={magicLink ? handleMagicLink : handlePasswordLogin} className="flex flex-col gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="input-field w-full pl-9 text-[var(--color-midnight)] bg-white/50"
                />
              </div>
            </div>
            ...
            <button type="submit" disabled={loading} className="btn-gold w-full mt-2 h-11 flex items-center justify-center gap-2">
              {loading ? "Please wait…" : magicLink ? "Send Magic Link" : "Sign In"}
            </button>
          </form>
          ...
          <button
            ...
            className="mt-4 text-xs flex items-center justify-center gap-1.5 w-full hover:underline"
            style={{ color: "rgba(45, 41, 38, 0.55)" }}
          >
            <Sparkles size={12} />
            {magicLink ? "Use password instead" : "Email me a magic link instead"}
          </button>

          <p className="text-xs text-center mt-6" style={{ color: "rgba(45, 41, 38, 0.55)", fontFamily: "var(--font-body)" }}>
            New here?{" "}
            <Link href={`/signup?next=${encodeURIComponent(next)}`} className="font-semibold hover:underline" style={{ color: "var(--color-gold)" }}>
              Create an account
            </Link>
          </p>
        </div>
